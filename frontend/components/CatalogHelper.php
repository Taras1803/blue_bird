...
class CatalogHelper
{
    static function getAllCatalogData()
    {
        ...
        $pagination = new Pagination([
            'defaultPageSize' => Options::getBySlug('products_per_page'),
    ...
    static function getSingleProductAttributes($product_id)
    {
        $values = [];
        $lang = Lang::getCurrent();
        $product_to_attributes = ProductsToAttributes::find()->where(['product_id' => $product_id])->andWhere(['!=', 'attribute_id', 1])->orderBy(['attribute_id' => SORT_DESC])->all();
        foreach ($product_to_attributes as $attribute) {
            ...
            if ($attribute_name && $attribute_value) {
                ...
                if (isset($values[$attribute->attribute_id])) {
                    $values[$attribute->attribute_id]['value'] .= ',' . $attribute_value->{$lang->url};
                } else {
                    $values[$attribute->attribute_id] = [
                        'attribute_name' => $attribute_name->{$lang->url},
                        'value' => $attribute_value->{$lang->url},
                    ];
                }
            }
        }
    ...
    static function getProductData($slug)
    {
        ...
        $categorySlug = Categories::find()
            ->leftJoin('sm_products_to_categories', '`sm_categories`.`id` = `sm_products_to_categories`.`category_id`')
            ->where(['product_id' => $product->id])->one()->slug;
        if ($product) {
            ...
            $data = [
                ...
                'lang' => $lang->url,
                'per_count' => ($product->type == 1)? '1' . Yii::t('main', 'pc') : '50' . Yii::t('main', 'gr'),
                'consist' => $consist,
